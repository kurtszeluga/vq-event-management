...
| Split oversized components and services | In Progress | `RegisterPage` step 0 done: derived eligibility and verification gates extracted to `src/utils/registrationEligibility.js` and covered by tests. The remaining problem is state, not length - `RegisterPage()` still holds 47 `useState` across five domains (event loading, registrant form, identity verification, payment/seat hold, submission). Planned order: event loading, registrant form, identity verification, then payment/seat hold last. Add a client test runner (vitest + testing-library) before the payment hook; there is currently no automated coverage of client behavior. EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
