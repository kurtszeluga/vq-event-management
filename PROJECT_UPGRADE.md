...
| Split oversized components and services | In Progress | `RegisterPage`: step 0 extracted derived eligibility/verification gates to `src/utils/registrationEligibility.js`. Step 1 extracted event loading to `src/hooks/useEventRegistration.js`. Step 2 extracted the registrant contact/billing form to `src/hooks/useRegistrantForm.js`; `RegisterPage.jsx` is now 1902 lines, down from 1949. Remaining domains still inline in `RegisterPage()`: identity verification, payment/seat hold (highest risk - has the re-entrancy lock and the hold-invalidation effect), submission. `npm run dev` (plain Vite) cannot verify anything past the email-lookup step, since `/api/*` routes are Vercel serverless functions it does not serve (confirmed: lookup returns 404, handled gracefully with no console errors) - `vercel dev` would serve them but pulls real Firebase/Resend secrets and would send live email and write live Firestore documents, so it was not run without asking first. This means identity verification and payment/seat hold cannot be manually browser-verified the same way steps 0-2 were; add a client test runner (vitest + testing-library) with the API layer mocked before touching either. EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
