...
| Split oversized components and services | In Progress | `RegisterPage`: step 0 extracted derived eligibility/verification gates to `src/utils/registrationEligibility.js`. Step 1 extracted event loading to `src/hooks/useEventRegistration.js`. Step 2 extracted the registrant contact/billing form to `src/hooks/useRegistrantForm.js` and added a vitest + Testing Library client test runner (`npm run test:client`, `tests/client/`, wired into CI) with 8 tests for the new hook. `RegisterPage.jsx` is now 1902 lines, down from 1949. Remaining domains still inline in `RegisterPage()`: identity verification, payment/seat hold (highest risk - has the re-entrancy lock and the hold-invalidation effect), submission - both now have a test harness available before extraction. EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
