...
| Split oversized components and services | In Progress | `RegisterPage`: step 0 extracted derived eligibility/verification gates to `src/utils/registrationEligibility.js`. Step 1 extracted event loading to `src/hooks/useEventRegistration.js`. Step 2 extracted the registrant contact/billing form to `src/hooks/useRegistrantForm.js` and added a vitest + Testing Library client test runner (`npm run test:client`, `tests/client/`, wired into CI). Step 3 extracted identity verification (email lookup, password sign-in, emailed-code fallback) to `src/hooks/useIdentityVerification.js` with 22 tests. `RegisterPage.jsx` is now 1747 lines, down from 1949; `useState` calls in the component are down from 54 to 22. Remaining domains still inline: payment/seat hold (highest risk - has the re-entrancy lock and the hold-invalidation effect) and submission. EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
