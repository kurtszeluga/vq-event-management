...
| Split oversized components and services | In Progress | `RegisterPage`: step 0 extracted derived eligibility/verification gates to `src/utils/registrationEligibility.js`. Step 1 extracted event loading and membership-settings subscription to `src/hooks/useEventRegistration.js` (4 states, 2 effects); `RegisterPage.jsx` is now 1910 lines, down from 1949. Remaining domains still inline in `RegisterPage()`: registrant form, identity verification, payment/seat hold (highest risk - has the re-entrancy lock and the hold-invalidation effect), submission. Add a client test runner (vitest + testing-library) before the payment hook; there is currently no automated coverage of client behavior, only manual browser verification. EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
