...
| Split oversized components and services | In Progress | `RegisterPage` domain extraction is complete. Step 0 moved derived eligibility/verification gates to `src/utils/registrationEligibility.js`; steps 1-4 moved event loading, the registrant form, identity verification, and payment/seat hold into `src/hooks/`. A vitest + Testing Library client runner (`npm run test:client`, `tests/client/`, in CI) covers the three stateful hooks with 54 tests. `RegisterPage.jsx` is 1600 lines, down from 1949, and its `useState` count is 14, down from 54 - what remains is orchestration (`handleSubmit`), submission state, and JSX. `RegistrationPaymentPanel` now lives in `src/components/RegistrationPaymentPanel.jsx` (409 lines) with its Square script-loading, config-validation, and payment-request helpers, leaving `RegisterPage.jsx` at 1194 lines. Optional remaining follow-up: extract submission state (`confirmation`, `formError`, `submitting`, `registrationFinalizing`, `handleSubmit`). EventForm, ConfigurationPanel, RegistrationPanel, UserControlPanel, and configurationService are untouched. |
